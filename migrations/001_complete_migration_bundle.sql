...
\echo '  ✓ companies'
\echo '  ✓ users'
\echo '  ✓ company_members'
\echo '  ✓ audit_logs'
\echo ''
\echo 'Added Columns:'
\echo '  ✓ companies.cnpj'
\echo '  ✓ z_api_instances.company_id'
\echo ''
\echo 'Created Master User:'
\echo '  ✓ Email: kairolopesoficial@gmail.com'
\echo '  ✓ Role: owner'
\echo '  ✓ Company: Master Company'
\echo ''
\echo 'Security Features:'
\echo '  ✓ Row Level Security (RLS) enabled on all tables'
\echo '  ✓ RLS policies configured for multi-tenant isolation'
\echo ''
\echo 'Run verification queries below to confirm migration success:'
\echo ''

-- Run verification queries
SELECT
  'VERIFICATION: All Tables Created' AS check_name,
...
SELECT
  'VERIFICATION: Indexes Created' AS check_name,
...
SELECT
  'VERIFICATION: Master User Created' AS check_name,
  COUNT(*) AS user_count,
  STRING_AGG(CONCAT(email, ' (', role, ')'), ', ') AS users
FROM users
