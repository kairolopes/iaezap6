-- ============================================================================
-- IAeZap Multi-Tenant System - Part 1: CREATE CORE TABLES
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS users (
  ...
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
  deleted_at TIMESTAMP WITH TIME ZONE,
  CONSTRAINT users_email_company_unique UNIQUE (company_id, email) DEFERRABLE INITIALLY DEFERRED
);
...
CREATE INDEX IF NOT EXISTS idx_company_members_invited_by ON company_members(invited_by);
CREATE INDEX IF NOT EXISTS idx_company_members_joined_at ON company_members(joined_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_companies_cnpj ON companies(cnpj) WHERE deleted_at IS NULL AND cnpj != '';
CREATE INDEX IF NOT EXISTS idx_companies_owner_id_active ON companies(owner_id) WHERE deleted_at IS NULL AND status = 'active';
