/**
 * JWT Claims Validation Test Script
 * Tests JWT claims from successful logins and validates all required claims
 */
...
const REQUIRED_CLAIMS = {
  ...
  role: {
    ...
    validator: (val) => typeof val === 'string' || Array.isArray(val),
  },
  iat: {
...
/**
 * Test the login endpoint and validate JWT claims
 */
async function testJWTClaims() {
  log(`\n${'='.repeat(80)}`, 'bold');
  log('JWT CLAIMS VALIDATION TEST', 'bold');
  log(`${'='.repeat(80)}\n`, 'bold');

  log(`Testing JWT endpoint: ${API_BASE_URL}/api/auth/login`, 'cyan');
  log(`Test user email: ${TEST_USER.email}\n`, 'cyan');

  try {
    // Step 1: Attempt login
    log('Step 1: Attempting login...', 'blue');
    log('-'.repeat(80), 'blue');
    ...
    if (!loginResponse.ok) {
      log(`Login failed with status ${loginResponse.status}:`, 'yellow');
      log(JSON.stringify(loginData, null, 2), 'yellow');

      // Suggest trying registration if user not found
      if (loginData.error?.code === 'INVALID_CREDENTIALS') {
        log('\nAttempting to register new test user...', 'yellow');
        ...
        if (!registerResponse.ok) {
          log(`Registration failed with status ${registerResponse.status}:`, 'red');
          log(JSON.stringify(registerData, null, 2), 'red');
          return;
        }

        log('User registered successfully!', 'green');
        log(`Registered user: ${registerData.user.id}`, 'green');

        // Use the token from registration
        if (registerData.token) {
          loginData.access_token = registerData.token.accessToken;
          loginData.refresh_token = registerData.token.refreshToken;
          loginData.user = registerData.user;
          loginData.company_id = registerData.user.company_id;
        } else if (registerData.access_token) {
          loginData.access_token = registerData.access_token;
          loginData.refresh_token = registerData.refresh_token;
          loginData.user = registerData.user;
          loginData.company_id = registerData.company_id;
        }
      } else {
        return;
      }
    ...
    // Step 2: Extract and decode access token
    log('\nStep 2: Decoding JWT access token...', 'blue');
    log('-'.repeat(80), 'blue');
    ...
    log('\nStep 3: Verifying JWT signature...', 'blue');
    log('-'.repeat(80), 'blue');
    ...
    // Step 4: Validate individual claims
    log('\nStep 4: Validating JWT claims...', 'blue');
    log('-'.repeat(80), 'blue');
    ...
    // Step 5: Display detailed claim validation results
    log('\nStep 5: Detailed Claim Analysis', 'blue');
    log('-'.repeat(80), 'blue');
      ...
      if (!result.present) {
        ...
        if (result.expiresInSeconds !== undefined) {
          ...
          if (result.expiryMatch) {
            ...
            log(`  Expiry time: MISMATCH (diff: ${Math.abs(result.expiresInSeconds - result.expectedExpirySeconds)} seconds)`, 'yellow');
          }
        }
    ...
    log('\nStep 6: Testing Refresh Token...', 'blue');
    log('-'.repeat(80), 'blue');
    ...
    log(`  Role: ${loginData.user.role} -> role/roles: ${JSON.stringify(payload.roles || payload.role)}`, 'cyan');
