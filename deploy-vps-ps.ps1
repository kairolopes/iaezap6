# ==============================================================================
# IAeZap - VPS Deployment via SSH (PowerShell)
# ==============================================================================

$ErrorActionPreference = "Stop"

$VPS_IP = "179.198.102.88"
$VPS_USER = "root"
$VPS_PASSWORD = "Bate123ria@5"
$DOMAIN = "jotaonline.com.br"
$EMAIL = "kairo@zapbaratinho.com.br"

Write-Host "🚀 IAeZap - VPS Deployment" -ForegroundColor Green
Write-Host "=" * 50 -ForegroundColor Green
Write-Host ""

# Try to use ssh.exe if available
try {
    Write-Host "📍 Testing SSH connectivity..." -ForegroundColor Cyan
    $result = & ssh.exe -o ConnectTimeout=5 -o StrictHostKeyChecking=no `
        -o UserKnownHostsFile=/dev/null $VPS_USER@$VPS_IP "echo OK" 2>&1

    if ($LASTEXITCODE -ne 0) {
        Write-Host "❌ SSH without password auth failed" -ForegroundColor Red
        Write-Host "   Trying alternative method..." -ForegroundColor Yellow
        throw "SSH failed"
    }
} catch {
    Write-Host "⚠️  SSH not available, using alternative method" -ForegroundColor Yellow
}

# Alternative: Create a deployment package and instructions
Write-Host ""
Write-Host "📝 Since SSH with password requires interactive auth," -ForegroundColor Yellow
Write-Host "   here are the manual deployment commands:" -ForegroundColor Yellow
Write-Host ""

$DeploymentCommands = @"
# Execute these commands via SSH in Hostinger Terminal or Web Console:

ssh root@179.198.102.88

# Then run:

cd /var/www && rm -rf iaezap; git clone https://github.com/KairoLopes/iaezap.git iaezap && cd iaezap

# Update system
apt-get update && apt-get upgrade -y

# Install Node.js
curl -fsSL https://deb.nodesource.com/setup_18.x | bash - && apt-get install -y nodejs

# Install tools
apt-get install -y git nginx certbot python3-certbot-nginx ufw && npm install -g pm2

# Create .env.production (will be prompted to edit)
nano .env.production

# Paste this content and save:
NEXT_PUBLIC_SUPABASE_URL=https://gqromcfhiosfppqlottz.supabase.co
SUPABASE_SERVICE_ROLE_KEY=sb_secret_uh1cDxnWtzRT4fhYbiEWBg_fbnZkKwQ

JWT_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\"

JWT_PUBLIC_KEY="-----BEGIN PUBLIC KEY-----\\ntQIDAQAB\n-----END PUBLIC KEY-----"

JWT_ISSUER=iaezap
JWT_AUDIENCE=iaezap-api
JWT_ACCESS_TOKEN_EXPIRY=3600
JWT_REFRESH_TOKEN_EXPIRY=604800
BCRYPT_ROUNDS=12
NODE_ENV=production
PORT=3000
NEXT_PUBLIC_APP_URL=https://jotaonline.com.br

# Save: Ctrl+X then Y then ENTER

# Continue deployment:
npm install
npm run build

# Start with PM2
pm2 start npm --name "iaezap" -- start
pm2 startup
pm2 save

# Configure Nginx
nano /etc/nginx/sites-available/jotaonline.com.br

# Paste this:
server {
    listen 80;
    listen [::]:80;
    server_name jotaonline.com.br www.jotaonline.com.br;
    return 301 https://\$server_name\$request_uri;
}

server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name jotaonline.com.br www.jotaonline.com.br;

    ssl_certificate /etc/letsencrypt/live/jotaonline.com.br/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/jotaonline.com.br/privkey.pem;

    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host \$host;
        proxy_cache_bypass \$http_upgrade;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
    }

    location /_next/static {
        alias /var/www/iaezap/.next/static;
        expires 30d;
        add_header Cache-Control "public, max-age=2592000, immutable";
    }
}

# Save: Ctrl+X then Y then ENTER

# Activate Nginx
ln -sf /etc/nginx/sites-available/jotaonline.com.br /etc/nginx/sites-enabled/
rm -f /etc/nginx/sites-enabled/default
nginx -t
systemctl restart nginx

# Generate SSL
certbot certonly --standalone --non-interactive --agree-tos -m kairo@zapbaratinho.com.br -d jotaonline.com.br -d www.jotaonline.com.br
systemctl restart nginx

# Setup Firewall
apt-get install -y ufw
ufw default deny incoming
ufw default allow outgoing
ufw allow 22/tcp
ufw allow 80/tcp
ufw allow 443/tcp
ufw --force enable

# Verify
pm2 status
pm2 logs iaezap

# Done! Configure DNS and wait 15-30 minutes for propagation

"@

Write-Host $DeploymentCommands -ForegroundColor Gray
Write-Host ""
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host "📝 STEPS:" -ForegroundColor Green
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
Write-Host "1️⃣  Copy the commands above" -ForegroundColor Yellow
Write-Host ""
Write-Host "2️⃣  Connect to VPS:" -ForegroundColor Yellow
Write-Host "    Option A: Use Hostinger Web Console" -ForegroundColor Cyan
Write-Host "    Option B: Use PowerShell: ssh root@179.198.102.88" -ForegroundColor Cyan
Write-Host "    Password: Bate123ria@5" -ForegroundColor Cyan
Write-Host ""
Write-Host "3️⃣  Paste commands one at a time" -ForegroundColor Yellow
Write-Host ""
Write-Host "4️⃣  When prompted to edit .env.production:" -ForegroundColor Yellow
Write-Host "    - Press 'i' for insert mode" -ForegroundColor Cyan
Write-Host "    - Paste the credentials from above" -ForegroundColor Cyan
Write-Host "    - Press ESC then type ':wq' and ENTER" -ForegroundColor Cyan
Write-Host ""
Write-Host "5️⃣  After deployment:" -ForegroundColor Yellow
Write-Host "    - Configure DNS at Hostinger panel" -ForegroundColor Cyan
Write-Host "    - Point jotaonline.com.br to IP: 179.198.102.88" -ForegroundColor Cyan
Write-Host ""
Write-Host "6️⃣  Wait 15-30 minutes for DNS propagation" -ForegroundColor Yellow
Write-Host ""
Write-Host "7️⃣  Access https://jotaonline.com.br/login" -ForegroundColor Yellow
Write-Host ""
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host "✅ System ready for deployment!" -ForegroundColor Green
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Green
